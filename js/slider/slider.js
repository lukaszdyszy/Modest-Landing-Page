class Slider
{
  ...
  changePeg(){
    for(let i=0; i<this.slide_number; i++){
      this.pegCnt.querySelectorAll('ul.slide-peg > li')[i].style.backgroundColor='rgba(255, 255, 255, 0)';
    }
    this.pegCnt.querySelectorAll('ul.slide-peg > li')[this.slide_nr].style.backgroundColor='rgba(255, 255, 255, 1)';
  }
  ...
  constructor(args = {}){
    ...
    if(this.draggable){
      ...
      this.handler.addEventListener('pointerup', function(){
        ...
        if(mouseEnd > mouseStart && mouseEnd-mouseStart > (obj.slide_width*0.15) && obj.slide_nr > 0){obj.slidePrev();}
        else if(mouseEnd < mouseStart && mouseStart-mouseEnd > (obj.slide_width*0.15) && obj.slide_nr < obj.slide_number-1){obj.slideNext();}
    ...
    if(this.peg){
      ...
      for(let i=0; i<obj.slide_number; i++){
        if(obj.pegStyle == 'dotted'){pegs += '<li></li>';}
    ...
    window.addEventListener('resize', function(){
      obj.slide_width = obj.handler.querySelector('.slide').getBoundingClientRect().width;
      document.querySelector(obj.alias + '> .slider-wrap > .slides').style.width = (obj.slide_number * obj.slide_width) + 'px';
      document.querySelector(obj.alias + '> .slider-wrap').style.width = obj.slide_width + 'px';
      obj.changeSlide()
    });
